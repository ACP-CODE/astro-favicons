...
export function logInfo(logs: ManifestLog[], totalTime: number, url: URL) {
  ...
  logs.forEach((log, idx) => {
    ...
    console.log(`${log.startTime}   \x1b[34m${symbol}\x1b[0m \x1B[2m/${log.filePath} (${log.excutionTime}ms)\x1B[22m`)
  });
...
): Promise<{ manifestLogs: ManifestLog[]; totalTime: number; }> {
